...
class Constants(cfg.Constants):
	...
	PATH_ROOT = cfg.Constants.API_PATH + cfg.Constants.DEFAULT_VERSION
...
class TestEndpoint(unittest.TestCase):
	...
	def test_03_put(self):
		...
		request = ft3.api.Request(
			url=url, path=url, method=Constants.PUT, body=response.body
		)
	...
	def test_05_get(self):
		...
		self.assertEqual(
			response.body[0]['type'], template.pkg.enm.PetType.dog.value
		)
	...
	def test_07_sub_patch(self):
		...
		request = ft3.api.Request(
			...
			body=template.pkg.obj.Pet(
				name='Gak', pet_with_pet_id=response.body['id']
			).to_dict(
				camel_case=True,
				include_null=False,
				include_private=False,
	...
	def test_08_sub_delete_get(self):
		...
		request = ft3.api.Request(url=url, path=url, method=Constants.DELETE)

		response = self.handler(request)

		request = ft3.api.Request(url=url, path=url, method=Constants.GET)
	...
	def test_10_sub_put_sophie(self):
		...
		request = ft3.api.Request(
			...
			method=Constants.POST,
			body=template.pkg.obj.Pet(
				name='Sophie', pet_with_pet_id=response.body['id']
			).to_dict(
				camel_case=True,
				include_null=False,
				include_private=False,
	...
	def test_16_options(self):
		...
		request = ft3.api.Request(url=url, path=url, method=Constants.OPTIONS)
	...
	def test_17_not_implemented(self):
		...
		request = ft3.api.Request(url=url, path=url, method='NOT_IMPLEMENTED')
