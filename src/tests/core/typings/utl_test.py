...
class TestUtils(unittest.TestCase):
	...
	def test_02_str_to_type(self):
		...
		self.assertIs(
			ft3.core.typ.utl.hint.resolve_type('int', globals(), locals()), int
		)
	...
	def test_04_arg_ref_handling(self):
		...
		self.assertIs(
			ft3.core.typ.utl.hint.resolve_type(
				'Mockery[tuple[int, ...]]', globals(), locals()
			),
			Mockery[tuple[int, ...]],
		)
	...
	def test_06_anti_is_variadic_array_type(self):
		...
		self.assertFalse(ft3.core.typ.utl.check.is_variadic_array_type(None))
	...
	def test_14_is_object(self):
		...
		self.assertTrue(ft3.core.typ.utl.check.is_object(mocking.Derivative))

	def test_15_is_field_type_forward_str(self):
		"""Test `is_field_type` works with `str` ForwardRef."""

		self.assertTrue(ft3.core.typ.utl.check.is_field_type('Field[Any]'))

	def test_16_is_immutable_type(self):
		"""Test `is_immutable_type`."""

		self.assertTrue(
			ft3.core.typ.utl.check.is_immutable_type(tuple[str, ...])
		)
