...
class TestUtils(unittest.TestCase):
	...
	def test_01_tuple_parse(self):
		...
		self.assertEqual(
			...
			codecs.utl.parse(
				codecs.lib.json.dumps(Constants.SimpleTuple), tuple[int, ...]
			),
		)
	...
	def test_17_anti_known_tuple_parse(self):
		...
		self.assertEqual(
			...
			codecs.utl.parse(
				codecs.lib.json.dumps(Constants.SimpleTuple), tuple[int, int]
			),
		)
	...
	def test_19_list_parse(self):
		...
		self.assertEqual(
			...
			codecs.utl.parse(
				codecs.lib.json.dumps(Constants.SimpleTuple), list[int]
			),
		)
	...
	def test_21_anti_list_parse(self):
		...
		self.assertEqual(
			codecs.enm.ParseErrorRef.invalid_arr_decode,
			codecs.utl.parse(
				codecs.lib.json.dumps(Constants.SimpleTuple), list[frozenset]
			),
		)
		...
		self.assertEqual(
			...
			codecs.utl.parse(
				codecs.lib.json.dumps(
					{
						i: v
						for (i, v) in enumerate(Constants.SimpleDict.values())
					}
				),
	...
	def test_25_anti_dict_parse(self):
		...
		self.assertEqual(
			...
			codecs.utl.parse(
				codecs.lib.json.dumps(
					{k: i for (i, k) in enumerate(Constants.SimpleDict.keys())}
				),
				dict[str, tuple[int, ...]],
			),
		)
	...
	def test_26_anti_dict_parse(self):
		...
		self.assertEqual(
			...
			codecs.utl.parse(
				codecs.lib.json.dumps(Constants.SimpleDict), dict[str]
			),
		)
	...
	def test_30_datetime_parse(self):
		...
		self.assertEqual(
			Constants.DateTime,
			codecs.utl.parse(
				Constants.DateTime.isoformat(), lib.datetime.datetime
			),
		)
	...
	def test_31_date_parse(self):
		...
		self.assertEqual(
			Constants.Date,
			codecs.utl.parse(Constants.Date.isoformat(), lib.datetime.date),
		)
	...
	def test_33_parse_literal(self):
		...
		self.assertEqual(
			...
			codecs.utl.parse(
				codecs.lib.json.dumps(Constants.SimpleTuple),
				tuple[lib.t.Literal[1], lib.t.Literal[2], lib.t.Literal[3]],
			),
		)
	...
	def test_34_anti_parse_literal(self):
		...
		self.assertEqual(
			...
			codecs.utl.parse(
				codecs.lib.json.dumps(Constants.SimpleTuple),
				tuple[lib.t.Literal[3], lib.t.Literal[2], lib.t.Literal[1]],
			),
		)
	...
	def test_35_parse_another_literal(self):
		...
		self.assertEqual(1, codecs.utl.parse('1', lib.t.Literal[1]))
	...
	def test_37_parse_typed_obj(self):
		...
		self.assertEqual(
			Constants.SimpleObj,
			codecs.utl.parse(
				codecs.lib.json.dumps(Constants.SimpleObj), SimpleTypedObj
			),
		)
	...
	def test_41_encode_unknown(self):
		...
		self.assertEqual(
			repr(UnknownSerializable), codecs.utl.encode(UnknownSerializable)
		)
