...
class TestObjectBase(unittest.TestCase):
	...
	def setUp(self) -> None:
		...
		self.anti = mocking.AntiTripDeriv(str_field='321')
		return super().setUp()
	...
	def test_02_dict_functionality(self):
		...
		self.assertRaises(
			KeyError, lambda: self.object_['field_that_does_not_exist']
		)
	...
	def test_12_to_dict(self):
		...
		self.assertDictEqual(
			...
			{
				k: v
				for k, v in self.trip.to_dict(camel_case=True).items()
				if type(v) in lib.t.get_args(ft3.core.typ.Primitive)
			},
		)
...
class TestObject(unittest.TestCase):
	...
	def test_02_dunder_get_ins(self):
		...
		self.assertIsInstance(self.object_.str_field, self.cls.str_field.type_)
	...
	def test_06_delitem(self):
		...
		self.assertEqual(self.cls.str_field.default, self.object_.str_field)
	...
	def test_08_setitem(self):
		...
		self.assertRaises(
			KeyError,
			lambda: self.object_.__setitem__('field_that_does_not_exist', 1),
		)
	...
	def test_15_pop(self):
		...
		self.assertRaises(
			KeyError, lambda: self.object_.pop('not_a_real_field')
		)
...
class TestObjectDocumentationExamples(unittest.TestCase):
	...
	def test_05_dict_functionality(self):
		...
		self.assertTrue(
			getattr(object_, key) == self.new_name != self.instance_values[key]
		)
	...
	def test_06_dict_keys(self):
		...
		self.assertListEqual(
			sorted(self.object_.keys()), sorted(self.instance_values)
		)
	...
	def test_07_dict_values(self):
		...
		self.assertListEqual(
			list(self.object_.values()), list(self.instance_values.values())
		)
	...
	def test_08_dict_items(self):
		...
		self.assertListEqual(
			sorted(self.object_.items()), sorted(self.instance_values.items())
		)
	...
	def test_09_dict_key_error(self):
		...
		self.assertRaises(
			...
			lambda: self.object_.setdefault(
				'field_that_does_not_exist', self.new_name
			),
		)
