	...
	def test_07_field_set_type_error(self):
		...
		self.assertRaises(
			...
			lambda: ft3.objects.metas.Meta.__setitem__(
				ft3.Field, 'name', ft3.Field(name='name', type_=int)
			),
		)
...
class TestFieldValidation(unittest.TestCase):
	...
	def test_04_parse(self):
		...
		self.assertIsInstance(
			self.cls.decimal_field.parse('4.5'), self.cls.decimal_field.type_
		)
	...
	def test_11_parse(self):
		...
		self.assertEqual(
			self.cls.dict_field.parse(
				json.dumps(self.cls.generic_dict_field.default)
			),
	...
	def test_13_parse(self):
		...
		self.assertEqual(
			self.cls.tuple_field.parse(
				json.dumps(self.cls.generic_tuple_field.default)
			),
