	...
	def test_07_print(self):
		...
		with self.assertWarns(Warning) as emitter:
			with self.assertNoLogs(self.log):
				print(msg)
				self.assertEqual(
					emitter.warnings[0].message.args[0], Constants.WARN_MSG
				)
...
class TestDeployedLogger(unittest.TestCase):
	...
	def test_02_print(self):
		...
		with self.assertWarns(Warning) as emitter:
			with self.assertNoLogs(self.log):
				print(msg)
				self.assertEqual(emitter.warnings[0].message.args[0], warn_msg)
	...
	def test_07_log_redaction(self):
		...
		with self.assertLogs(self.log, level) as logger:
			ft3.log.info(msg)
			self.assertIn(
				'[ REDACTED :: API_KEY_TOKEN ]', logger.records[0].msg
			)
			...
			**kwargs: lib.t.Any,
			...
			if (
				...
				if 'printed' in msg_dict:
					msg_final = ft3.loggers.typ.LogRecordWithPrintAndTraceBack(
